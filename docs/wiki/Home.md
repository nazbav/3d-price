...
Дополнительные возможности включают импорт/экспорт JSON, синхронизацию через Supabase и генерацию этикеток.

В каталоге `modern` размещена полнофункциональная версия на основе AdminLTE, повторяющая возможности `test.html`.
