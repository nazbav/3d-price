...
Экспериментальная версия также доступна как Electron‑приложение с хранением
данных в SQLite. Запустите его командой `npm start путь/к/файлу.gcode`
из каталога `electron-app` — указанный G-code будет импортирован автоматически.
...
> **Примечание.** Для корректного отображения интерфейса `test.html` теперь загружает стили Bootstrap напрямую из CDN.
