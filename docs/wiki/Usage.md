...
1. Откройте `index.html` в браузере или перейдите по ссылке из README.
...
19. Исправлено отображение окон связывания профилей — добавлен закрывающий тег в `index.html`.
...
23. На страницу index.html можно перетащить подготовленные gcode-файлы (созданные через `open_calc.py`). Модели добавятся автоматически.
...
31. Раздел «Мой Налог» теперь использует актуальные API `https://lknpd.nalog.ru`. Авторизация проходит через SMS‑челлендж (endpoint `api/v2`) и хранит refresh‑токен для автоматического продления сессии. Все URL собираются из переменной `MY_NALOG_API` в `index.html`.
