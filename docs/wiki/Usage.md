...
> Интерфейс использует стили Bootstrap, загружаемые из CDN. Для работы без интернета необходимо иметь кэшированную версию страницы.
...
23. На страницу test.html можно перетащить подготовленные gcode-файлы или передать их путь в командной строке при запуске Electron. Модели добавятся автоматически.
...
45. Калькулятор можно запустить как настольное приложение через Electron. Вызов
    `npm start path/to/file.gcode` в каталоге `electron-app` импортирует модель и
    сохранит данные в SQLite.
