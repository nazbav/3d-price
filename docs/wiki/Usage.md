...
8. В подразделе **Spoolman** введите адрес сервера и API‑ключ. Кнопка «Синхронизировать» работает через REST‑API v1 (`/api/v1/spool`) и обновляет остатки материалов с отправкой изменений обратно.
