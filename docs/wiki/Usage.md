...
31. Раздел «Мой Налог» теперь использует актуальные API `https://lknpd.nalog.ru`. Авторизация проходит через SMS‑челлендж и хранит refresh‑токен для автоматического продления сессии.
