...
37. В брендинге можно выбрать способ оплаты: по реквизитам или по СБП. В счёте QR-код генерируется для выбранного метода.
38. Кнопки запроса и подтверждения SMS в разделе «Мой Налог» больше не выполняют двойных запросов.
39. Поле реквизитов переименовано в «Банковские реквизиты (р/с)», QR-код для СБП использует корректный адрес.
40. При выборе СБП QR ведёт на чат поддержки, повторно сформировать чек нельзя.
