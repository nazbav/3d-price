...
```bash
cd electron-app
npm start path/to/file.gcode
```
Electron загрузит `test.html`, импортирует переданный G-code и сохранит данные в SQLite.
...
Данные настольной версии сохраняются в SQLite через `eStorage` и состоят из нескольких блоков.
...
- Все данные в режиме Electron хранятся в базе SQLite и не требуют подключения к серверу.
...
Кнопка переключения темы находится в шапке каждой страницы. При запуске через Electron выбранная тема сохраняется в SQLite.
