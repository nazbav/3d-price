...
### Загрузка зависимостей для офлайн-использования

Для работы без интернета можно загрузить все внешние зависимости локально:

```bash
# Загрузить зависимости для test.html в папку deps
python scripts/download_deps_recursive.py test.html

# Загрузить зависимости в пользовательскую папку
python scripts/download_deps_recursive.py test.html my_deps

# Посмотреть справку
python scripts/download_deps_recursive.py --help
```

Скрипт автоматически:
- Парсит HTML-файл и находит все внешние CSS/JS зависимости
- Загружает их с сохранением структуры папок (`deps/css`, `deps/js`, `deps/assets`)
- Рекурсивно обрабатывает CSS-файлы для поиска `@import` и `url()` ресурсов
- Избегает повторных загрузок одинаковых файлов
- Ведёт подробные логи в файле `download_deps.log`
