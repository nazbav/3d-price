...
- добавлена минимальная обёртка Electron с SQLite. Приложение запускает test.html и принимает G-code в параметрах
- test.html работает в Electron с SQLite, добавлена кнопка синхронизации Orca
- хранилище LocalStorage заменено на SQLite через объект eStorage
- добавлены IPC-методы remove-data
- реализована функция очистки БД и отказ от fallback на LocalStorage
- исправлена ошибка в стилях test.html, функции eStorage теперь возвращают
  промисы и логируют сбои, обновлена документация
- для восстановления внешнего вида страница снова использует Bootstrap из CDN
- удалены локальные файлы стилей, страница подключает Bootstrap и иконки через CDN
