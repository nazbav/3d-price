...
- экспериментальная версия на Electron с базой SQLite и импортом G-code через параметры
- localStorage синхронизируется через SQLite; кнопка Orca запускает sync_orca
- eStorage сохраняет все данные напрямую в SQLite, добавлена поддержка remove-data
- eStorage теперь работает только с SQLite и поддерживает полную очистку БД
- eStorage возвращает промисы, улучшена обработка ошибок, данные сохраняются
  в SQLite даже для темы оформления
- Bootstrap подключается через CDN для корректных стилей
- локальные стили убраны, используется CDN Bootstrap и Bootstrap Icons
